...
public class AseFileImporterEditor : ScriptedImporterEditor {
    ...
    private string[] spritePivotOptions = new string[] { "Center", "Top Left", "Top", "Top Right", "Left", "Right", "Bottom Left", "Bottom", "Bottom Right", "Custom" };
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.LabelField("Texture Options", EditorStyles.boldLabel);
        {
            ...
            importer.textureSettings.wrapMode = (TextureWrapMode)EditorGUILayout.EnumPopup("Wrap Mode", importer.textureSettings.wrapMode);
            importer.textureSettings.filterMode = (FilterMode)EditorGUILayout.EnumPopup("Filter Mode", importer.textureSettings.filterMode);
    ...
    private void PivotPopup(string label)
    {
        ...
        {
            case 0: customSpritePivot = false; pivot = new Vector2(0.5f, 0.5f); break;
            case 1: customSpritePivot = false; pivot = new Vector2(0f, 1f); break;
            case 2: customSpritePivot = false; pivot = new Vector2(0.5f, 1f); break;
            case 3: customSpritePivot = false; pivot = new Vector2(1f, 1f); break;
            case 4: customSpritePivot = false; pivot = new Vector2(0f, 0.5f); break;
            case 5: customSpritePivot = false; pivot = new Vector2(1f, 0.5f); break;
            case 6: customSpritePivot = false; pivot = new Vector2(0f, 0f); break;
            case 7: customSpritePivot = false; pivot = new Vector2(0.5f, 0f); break;
            case 8: customSpritePivot = false; pivot = new Vector2(1f, 0f); break;
            default: customSpritePivot = true; break;
        }
        if (customSpritePivot)
        {
            EditorGUI.indentLevel++;
            EditorGUILayout.PropertyField(serializedObject.FindProperty("textureSettings.spritePivot"), new GUIContent(label));
            EditorGUI.indentLevel--;
        }
    ...
    private void DrawAnimationSetting(AseFileImporter importer, AseFileAnimationSettings setting)
    {
        ...
        if (foldoutStates[setting.animationName] = EditorGUILayout.Foldout(foldoutStates[setting.animationName], setting.animationName, true, foldoutStyle))
        {
